...
Este documento descreve os **15 cenários de teste** (Testes 59–73) que validam a comissão por linha para **produtos**, cobrindo os tipos de linha **Diferenciada Por Vendedor** e **Mista**.
...
|---|---|---|---|---|
| **59** | `PROD__DIF_POR_VEND__COM_ALIQ` | Venda Balcão | ✅ Sim | `cpv.Aliquota` (> 0) |
| **60** | `PROD__DIF_POR_VEND__COM_ALIQ` | OS prod + serviço | ✅ Sim | `cpv.Aliquota` (> 0) |
| **61** | `PROD__DIF_POR_VEND__COM_ALIQ` | OS só produto | ✅ Sim | `cpv.Aliquota` (> 0) |
| **62** | `PROD__DIF_POR_VEND__SEM_ALIQ` | Venda Balcão | ❌ Não | `cpv.Aliquota` (= 0) |
| **63** | `PROD__DIF_POR_VEND__SEM_ALIQ` | OS prod + serviço | ❌ Não | `cpv.Aliquota` (= 0) |
| **64** | `PROD__DIF_POR_VEND__SEM_ALIQ` | OS só produto | ❌ Não | `cpv.Aliquota` (= 0) |
| **65** | `PROD__MISTA__COM_ALIQ` | Venda Balcão | ✅ Sim | `cpv.Aliquota` (> 0) |
| **66** | `PROD__MISTA__COM_ALIQ` | OS prod + serviço | ✅ Sim | `cpv.Aliquota` (> 0) |
| **67** | `PROD__MISTA__COM_ALIQ` | OS só produto | ✅ Sim | `cpv.Aliquota` (> 0) |
| **68** | `PROD__MISTA__COM_ALIQ_ZERO` | Venda Balcão | ❌ Não | `cpv.Aliquota` (= 0) |
| **69** | `PROD__MISTA__COM_ALIQ_ZERO` | OS prod + serviço | ❌ Não | `cpv.Aliquota` (= 0) |
| **70** | `PROD__MISTA__COM_ALIQ_ZERO` | OS só produto | ❌ Não | `cpv.Aliquota` (= 0) |
| **71** | `PROD__MISTA__SEM_REG_CPLV` | Venda Balcão | ✅ Sim | `cl.Aliquota` (geral mista) |
| **72** | `PROD__MISTA__SEM_REG_CPLV` | OS prod + serviço | ✅ Sim | `cl.Aliquota` (geral mista) |
| **73** | `PROD__MISTA__SEM_REG_CPLV` | OS só produto | ✅ Sim | `cl.Aliquota` (geral mista) |

---

## Variável de Controle: `Cenario_Sem_Comissao_Produto`

Nos cenários onde a comissão esperada é **zero** (Testes 62–64 e 68–70), a variável `${Cenario_Sem_Comissao_Produto}` é setada para `${True}` no `[Setup]` do teste. Isso faz com que:
...
| `Teste_Comissoes1.robot` | 15 novos casos de teste (Testes 59–73) |
